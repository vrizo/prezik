...
3. Director: one strong-model call. Input: brief + sitemap + options + user guidance. Output: a Storyboard (shared/src/storyboard.ts) including every narration line. The call is streamed: the model's reasoning summary and the scenes as they are drafted land in a plan_progress row (deleted once the storyboard is saved), so the Plan view shows live thinking and scene cards instead of a skeleton while it works. Retry and validation are unchanged — the whole stream is one attempt.
